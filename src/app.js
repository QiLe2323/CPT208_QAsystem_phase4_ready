...
function findDemoUser(username, password) {
  ...
  return DEFAULT_USERS.map(normalizeUser).find((user) => (
    ...
    try {
      if (state.backendReady) {
        try {
          ...
          const demoAccount = findDemoUser(username, password);
          if (!demoAccount) throw error;
          startSessionForUser(demoAccount);
        }
      } else {
